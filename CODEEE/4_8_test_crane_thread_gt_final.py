...
delta_time = 0
sampling_time = 0.01
# model = "/mah/AI/6-DOF-Inertial-Odometry_orignal/4_8_two_motion_win_50_dt_0.01_model_ep_800_data_filter.hdf5"
# model = "/mah/AI/6-DOF-Inertial-Odometry_orignal/4_8_two_motion_win_200_dt_0.01_model_ep_800_data_filter.hdf5"
model = "/mah/AI/6-DOF-Inertial-Odometry_orignal/crane_data/all_data_win_200_dt_01.hdf5"
...
def publish_lidar_tf_real(yhat_delta_p, yhat_delta_q):
  ...
  t.header.stamp = rospy.Time.now()
  t.header.frame_id = "base_link"
  t.child_frame_id = "front_laser_link_dt_gt"
  t.transform.translation.x =  yhat_delta_p[0]
  t.transform.translation.y =  yhat_delta_p[1]
  t.transform.translation.z =  yhat_delta_p[2]
...
def thread_filter_NN(gt_data, imu_data):
    ...
    [x_gyro, x_acc], [y_delta_p, y_delta_q], init_p, init_q = load_dataset_6d_quat_stride_last(gyro_data, acc_data, pos_data, ori_data, window_size, stride)
   
    if first_loop_flag == False or ijk == 3:
        init_p_first = init_p
        init_q_first = init_q
        gt_trj_p = init_p
    ...
    y_delta_q_euler = quaternion.as_euler_angles(quaternion.from_float_array(y_delta_q))
    print("y_delta ", np.array(y_delta_p), np.array(y_delta_q_euler)*180/math.pi , np.array(y_delta_q))
    ...
    yhat_delta_q_euler = quaternion.as_euler_angles(quaternion.from_float_array(yhat_delta_q))
    print("Yhat_delta  ", np.array(yhat_delta_p), np.array(yhat_delta_q_euler)*180/math.pi , np.array(yhat_delta_q))
    ...
    # pred_trj_q_euler = quaternion.as_euler_angles(pred_trj_q)
    # print("pred_trj_q_euler1 = ", np.array(pred_trj_q_euler)," ", np.array(quaternion.as_float_array(pred_trj_q)))
    # publish the rpy

    # yhat_delta_q1 = quaternion_from_euler(yhat_delta_q_euler[0], yhat_delta_q_euler[1] , yhat_delta_q_euler[2])
    # yhat_delta_q1_r = [yhat_delta_q1[3], yhat_delta_q1[0], yhat_delta_q1[1],yhat_delta_q1[2]]
    # yhat_delta_q1_euler = euler_from_quaternion(yhat_delta_q1)


    # pred_trj_q1 = quaternion.from_euler_angles(pred_trj_q_euler[0]*0, pred_trj_q_euler[1], pred_trj_q_euler[2])
    # # yhat_delta_q1 = quaternion.from_euler_angles(yhat_delta_q_euler)
    # pred_trj_q1_euler = quaternion.as_euler_angles(pred_trj_q1)
    # print("pred_trj_q_euler2 = ", np.array(pred_trj_q1_euler), " ", np.array(quaternion.as_float_array(pred_trj_q1)))
    # pred_trj_q = pred_trj_q1
    ...
    pred_q = quaternion.as_float_array(pred_trj_q)
    pred_q = ( pred_q[1], pred_q[2], pred_q[3], pred_q[0])
    pred_trj_q_euler = euler_from_quaternion(pred_q,  axes='sxyz')
    # print("cur_p_prd1 ", np.array(pred_trj_p_time), np.array(pred_trj_q_euler)*180/math.pi , np.array(pred_q))
    first_loop_flag = True
    pred_q1 = quaternion_from_euler(pred_trj_q_euler[0], pred_trj_q_euler[1] , pred_trj_q_euler[2]) # multiply 0 to x
    # yhat_delta_q1_r = [yhat_delta_q1[3], yhat_delta_q1[0], yhat_delta_q1[1],yhat_delta_q1[2]]
    pred_trj_q_euler1 = euler_from_quaternion(pred_q1)
    # print("cur_p_prd2 ", np.array(pred_trj_p_time), np.array(pred_trj_q_euler1)*180/math.pi , np.array(pred_q1))
    pred_q1 = [pred_q1[3], pred_q1[0], pred_q1[1], pred_q1[2]]
    pred_trj_q = quaternion.from_float_array(pred_q1)
...
if __name__ == '__main__':
    ...
    if (delta_time > duration) :
        ...
        plot_pos_error_gazebo("pred vs gt error " ,  pred_trj_p[1:nn,1:], pred_trj_q[1:nn,:], gt_slice_array[1:nn,1:4], gt_slice_array[1:nn,4:], n1, n2)

        
        # for i in range(0, len(pred_trj_p[1:nn,:])):
        from numpy import linalg as LA


        delta_pos_pred =   pred_trj_p[2:nn:2] - pred_trj_p[1:nn-1:2]  
        vel_pred = delta_pos_pred[:,1:]/delta_pos_pred[:,:1]
        vel_pred_norm = LA.norm(vel_pred, axis = 1)
        len(vel_pred_norm)


        delta_pos_gt =   gt_trj_p[2:nn:2] - gt_trj_p[1:nn-1:2]  
        vel_gt = delta_pos_gt[:,1:]/delta_pos_gt[:,:1]
        vel_gt_norm = LA.norm(vel_gt, axis = 1)
        len(vel_gt_norm)

        fig = plt.figure("speed change")
        plt.plot(vel_pred_norm, label = "pred")
        plt.plot(vel_gt_norm, label = "gt")
        plt.legend()
