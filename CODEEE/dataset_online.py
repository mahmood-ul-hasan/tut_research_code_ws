...
def load_dataset_6d_quat(gyro_data, acc_data, pos_data, ori_data, window_size, stride):
    ...
    for idx in range(0, gyro_data.shape[0], stride):
        #x.append(gyro_acc_data[idx + 1 : idx + 1 + window_size, :])
        x_gyro.append(gyro_data[idx : idx + 1 + window_size, :])
        x_acc.append(acc_data[idx : idx + 1 + window_size, :])

        p_a = pos_data[idx + window_size//2 - stride//2, :]
        p_b = pos_data[idx + window_size//2 + stride//2 - 1, :]

        q_a = quaternion.from_float_array(
            ori_data[idx + window_size//2 - stride//2 , :])
        q_b = quaternion.from_float_array(
            ori_data[idx + window_size//2 + stride//2 - 1, :])
