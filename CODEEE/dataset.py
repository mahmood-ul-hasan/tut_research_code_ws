...
def load_dataset_6d_quat(gyro_data, acc_data, pos_data, ori_data, window_size, stride):
    ...
    for idx in range(0, gyro_data.shape[0] - window_size - 1, stride):
        ...
        delta_p = np.matmul(
            quaternion.as_rotation_matrix(q_a).T, (p_b.T - p_a.T)).T
...
def load_dataset_6d_quat_stride_last(gyro_data, acc_data, pos_data, ori_data, window_size, stride):
    #gyro_acc_data = np.concatenate([gyro_data, acc_data], axis=1)

    init_p = pos_data[window_size - stride, :]
    init_q = ori_data[window_size - stride, :]
    ...
    for idx in range(0, gyro_data.shape[0] - window_size - 1, stride):
        ...
        p_a = pos_data[idx + window_size - stride, :]
        p_b = pos_data[idx + window_size , :]

        q_a = quaternion.from_float_array(
            ori_data[idx + window_size - stride, :])
        q_b = quaternion.from_float_array(
            ori_data[idx + window_size, :])
