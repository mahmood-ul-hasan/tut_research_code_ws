...
def plot_trj_error_vector(title, legend , error_traj):
    ...
    plt.subplot(311); plt.plot(error_traj[:,0], label=  str(legend)); plt.grid(True); plt.xlabel("Frame"); plt.ylabel("Trajectory Error x(m)");    plt.legend()
    plt.subplot(312); plt.plot(error_traj[:,1], label=  str(legend)); plt.grid(True); plt.xlabel("Frame"); plt.ylabel("Trajectory Error x(m)");    plt.legend()
    plt.subplot(313); plt.plot(error_traj[:,2], label=  str(legend)); plt.grid(True); plt.xlabel("Frame"); plt.ylabel("Trajectory Error x(m)");    plt.legend()
...
def plot_graphs_first(title, legend, init_p, init_q, y_delta_p, y_delta_q,yhat_delta_p, yhat_delta_q):
    ...
    plot_pos_error("dgt vs gt error " ,  pred_trajectory,  yhat_delta_q, gt_trajectory, y_delta_q, 0, len(y_delta_q))
...
def plot_sampling_rate_01():
    ...
    plot_graphs_first(title, legend,init_p, init_q, y_delta_p, y_delta_q,yhat_delta_p, yhat_delta_q)
    print(legend)
    ...
    class win_50_dt_01_filter:
        ...
        input = '/mah/AI/6-DOF-Inertial-Odometry_orignal/win50/4_1_simple_motion_win_26_dt_0.002_imu_data_filter.csv'
        gt = '/mah/AI/6-DOF-Inertial-Odometry_orignal/win50/4_1_simple_motion_win_26_dt_0.002_gt_data_filter.csv'
        window_size = 50
        stride = 26
        limit = 100*60
