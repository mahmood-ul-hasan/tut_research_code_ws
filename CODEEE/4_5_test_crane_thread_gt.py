...
def publish_lidar_tf_pred1(yhat_delta_p, yhat_delta_q):
...
def publish_lidar_tf_pred(yhat_delta_p, yhat_delta_q):
  ...
  # quat_boom_yaw = quaternion_from_euler(euler_boom[0], euler_boom[1] , euler_boom[2],)
  quat_boom_yaw = quaternion_from_euler(euler_boom[0]*0, euler_boom[1] , euler_boom[2],)
  # print("yhat_delta_q  = ", yhat_delta_q)
  # print("quat_boom_yaw = ", quat_boom_yaw)
...
def get_imu_data():
    ...
    while  delta_time1 <= duration and not rospy.is_shutdown():
     try:
        ...
        if now_time_stamp1 >=  pervious_time_1 +  (0.002):
...
def thread_filter_NN(gt_data, imu_data):
    ...
    # filtered_wx = noisy_wx
    # filtered_wy =noisy_wy
    # filtered_wz=noisy_wz

    # filtered_ax=noisy_ax
    # filtered_ay=noisy_ay
    # filtered_az= noisy_az
    ...
    gt_trj_q_euler = euler_from_quaternion(dgt_q, axes='rzyx')
...
if __name__ == '__main__':
    ...
    while  delta_time <= duration and not rospy.is_shutdown():
     try:
        ...
        if (len(imu_pose) > ii+ window_size):
            ...
            gt_slice_euler = euler_from_quaternion(gt_slice1, axes='rzyx')
            # gt_euler = (gt_slice[4:])
            ...
            imu_data = imu_data[ii :ii + window_size, :]
            ii = ii+ stride
    ...
    if (delta_time > duration) :
        ...
        # np.savetxt('/home/aisl/catkin_ws/src/latest_crane/imu_pose/data_gt_AI.csv', gt_data, delimiter=',', fmt = '%.9f')
        # np.savetxt('/home/aisl/catkin_ws/src/latest_crane/imu_pose/gt_cur_p.csv', gt_trj_p, delimiter=',', fmt = '%.9f')
        ...
        plot_pos_error("dgt vs gt error " ,  gt_trj_p[1:nn,1:], gt_trj_q[1:nn,:], gt_slice_array[1:nn,1:4], gt_slice_array[1:nn,4:], n1, n2)
