...
def load_euroc_mav_dataset(imu_data_filename, gt_data_filename):
    ...
    return gyro_data, acc_data, pos_data, ori_data, time
...
def load_dataset_6d_quat_absolute(gyro_data, acc_data, pos_data, ori_data, window_size=200, stride=10):
    #gyro_acc_data = np.concatenate([gyro_data, acc_data], axis=1)
    ...
    y_delta_p = []
    y_delta_q = []
    delta_p_t = np.zeros(3)
    q_t = quaternion.from_float_array(np.array([1, 0, 0, 0]))
    q_t2 = quaternion.from_float_array(np.array([1, 0, 0, 0]))
    q_T = quaternion.from_float_array(np.array([1, 0, 0, 0]))
    # euler_q_T2 = np.zeros((len(gyro_data),3))
    euler_q_T2 = np.zeros((len(gyro_data), 3))
    euler_delta_q2 = np.zeros((len(gyro_data), 3))
    y_euler_q_T2 = []

    for idx in range(0, gyro_data.shape[0] - window_size - 1, stride):
        #x.append(gyro_acc_data[idx + 1 : idx + 1 + window_size, :])
        # gyro_data[idx + 1 : idx + 1 + window_size, :]
        x_gyro.append(gyro_data[idx + 1: idx + 1 + window_size, :])
        x_acc.append(acc_data[idx + 1: idx + 1 + window_size, :])
        y_delta_p.append(pos_data[idx + 1: idx + 1 + window_size, :])
        y_delta_q.append(ori_data[idx + 1: idx + 1 + window_size, :])
        ...
        #     q_a = quaternion.from_float_array(ori_data[i, :])
        #     q_b = quaternion.from_float_array(ori_data[i+1, :])

        #     delta_p = (p_b.T - p_a.T).T
        #     delta_p_t = delta_p_t + delta_p
        ...
        #     delta_q2 = quaternion.as_float_array(delta_q)
        #     delta_q2 = np.concatenate([delta_q2[1:4], delta_q2[0:1]])
        #     euler_delta_q2[i,:] = euler_from_quaternion(delta_q2, axes='szyx')

        #     q_T = force_quaternion_uniqueness(q_t * delta_q)
        #     q_t =  q_T
        #     q_T2 = quaternion.as_float_array(q_T)
        #     q_T3 = np.concatenate([q_T2[1:4], q_T2[0:1]])
        #     euler_q_T2[i,:] = euler_from_quaternion(q_T3, axes='szyx')            # euler_q_T2[i, :] =

        # y_euler_q_T2.append(euler_q_T2[i,:])
        # y_delta_p.append(delta_p_t)
        # y_delta_q.append(quaternion.as_float_array(q_T))
        # delta_p_t =np.zeros(3)
        # q_T = quaternion.from_float_array(np.array([1, 0, 0, 0]))
        # q_t =   q_T
    ...
    x_gyro = np.reshape(x_gyro, (len(x_gyro), x_gyro[0].shape[0], x_gyro[0].shape[1]))
    x_acc = np.reshape(x_acc, (len(x_acc), x_acc[0].shape[0], x_acc[0].shape[1]))
    y_delta_p = np.reshape(y_delta_p, (len(y_delta_p), y_delta_p[0].shape[0], y_delta_p[0].shape[1]))
    y_delta_q = np.reshape(y_delta_q, (len(y_delta_q), y_delta_q[0].shape[0], y_delta_q[0].shape[1]))
...
def modified_load_dataset_6d_quat(gyro_data, acc_data, pos_data, ori_data, window_size=200, stride=10):
    #gyro_acc_data = np.concatenate([gyro_data, acc_data], axis=1)
    ...
    for idx in range(0, gyro_data.shape[0] - window_size - 1, window_size):
        #x.append(gyro_acc_data[idx + 1 : idx + 1 + window_size, :])
        gyro_data[idx + 1: idx + 1 + window_size, :]
        x_gyro.append(gyro_data[idx + 1: idx + 1 + window_size, :])
        x_acc.append(acc_data[idx + 1: idx + 1 + window_size, :])
