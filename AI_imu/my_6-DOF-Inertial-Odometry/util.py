...
def my_generate_trajectory_6d_quat(init_p, init_q, y_delta_p, y_delta_q):
    cur_p = np.array(init_p)
    cur_q = quaternion.from_float_array(init_q)
    pred_p = []
    pred_p.append(np.array(cur_p))
    pred_q = []
    pred_q.append(np.array(quaternion.as_float_array(cur_q)))
    for [delta_p, delta_q] in zip(y_delta_p, y_delta_q):
        cur_p = cur_p + np.matmul(quaternion.as_rotation_matrix(cur_q), delta_p.T).T
        cur_p = cur_p + delta_p.T
        cur_q = cur_q * quaternion.from_float_array(delta_q).normalized()
    ...
    print("pred_p", len(pred_p))
    print("pred_q", len(pred_q))
    print("pred_p", pred_p)
    print("pred_q", pred_q)

    print("np pred_p", np.reshape(pred_p, (len(pred_p), 3)))
    print("np pred_q", np.reshape(pred_q, (len(pred_q), 4)))
